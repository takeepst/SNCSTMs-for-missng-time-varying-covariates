###########################################################################
#Program name           : simfunc_data_nm.r
...
setwd("/SimulationCode/Non-monotone")
...
for (s in 1:n.reps){
  ...
  Mbasep <- 0 # Intercept of missing probability models 
  ...
  useML <- matrix(F, K+1, K+1)
  
  for (k in 1:K)
    useML[k+1,] <- c(rep(1, k-1), 1, rep(0, K-k+1))
  
  useAcensor <- matrix(F, K+1, K+1)
  
  for (k in 0:K)
    useAcensor[k+1,] <- c(rep(1, k+1), rep(0, K-k))
  
  source("generate_data_nm.r")
  ...
  res <- sncstm_NM(data=data, useA=useA, useL=useL, useAcensor=useAcensor, useML=useML, EXPOSETYPE=EXPOSETYPE, MISSTYPE=MISSTYPE,ESTIMATOR=ESTIMATOR,IPCW=IPCW,Mprob=Mprob,Mimp=Mimp,gamma=gamma)
