...
sncstm_NM <- function(data, indices=NULL, useA, useL, useML, useZ=NULL, EXPOSETYPE, MISSTYPE, NQUAD=10, IPCW=F, admincens=NULL, useAcensor=NULL, useLcensor=NULL, EXPONCUM=F, ESTIMATOR="DR", Mprob=F, Mimp=F, gamma=-0.2) {
  ...
  # Calculate the deltastars. deltastar <- array(0, c(N, K+1, K+1))
  ...
  for(t in 1:(K+1)){
    ...
    #missing probability model
    if (t > 1){
      tmpLb <- Lb[atrisk==1,(1:(t-1))]
      histM <- MLb[atrisk==1,(1:(t-1))]
      tmpLb[is.na(tmpLb)] <- 0
      if (Mprob==T){
        tmp <- cbind(1,tmpA,tmpV,tmpLa,tmpLb,histM,Lb[atrisk==1,t])
      }else{
        tmp <- cbind(1,tmpA,tmpV,tmpLa,tmpLb,histM,tmpF,tmpT,Lb[atrisk==1,t])
      }
    } else {
      if (Mprob==T){
        tmp <- cbind(1,tmpA,tmpV,tmpLa,Lb[atrisk==1,t])
      } else {
        tmp <- cbind(1,tmpA,tmpV,tmpLa,tmpF,tmpT,Lb[atrisk==1,t])
      }
    }
    ...
    while (diff >= crit )
    {
      ...
      if (t > 1){
        if (Mprob==T){
          tmp2 <- cbind(1,tmpA,tmpV,tmpLa,tmpLb,histM)[outM==0,]
        } else {
          tmp2 <- cbind(1,tmpA,tmpV,tmpLa,tmpLb,histM,tmpF,tmpT)[outM==0,]
        }
      } else {
        if (Mprob==T){
          tmp2 <- cbind(1,tmpA,tmpV,tmpLa)[outM==0,]
        } else {
          tmp2 <- cbind(1,tmpA,tmpV,tmpLa,tmpF,tmpT)[outM==0,]
        }
      }
      ...
      if (t > 1){
        if (Mprob==T) {
          tmp3 <- cbind(1,tmpA,tmpV,tmpLa,tmpLb,histM)[outM==1,]
        } else {
          tmp3 <- cbind(1,tmpA,tmpV,tmpLa,tmpLb,histM,tmpF,tmpT)[outM==1,]
        }
      } else{
        if (Mprob==T) {
          tmp3 <- cbind(1,tmpA,tmpV,tmpLa)[outM==1,]
        } else {
          tmp3 <- cbind(1,tmpA,tmpV,tmpLa,tmpF,tmpT)[outM==1,]
        }
      }
    ...
    wlist[[t]] <- cbind(data$id[data$tim >= (t-1)],estimates.mp)
    ...
    pooled <- pooled[pooled$tim>=(t-1),]
    indices2 <- 1:length(pooled$tim)
    tim2 <- as.matrix( pooled[indices2, grepl("tims.", names(pooled))] )
    tim2 <- tim2[,(t:(K+1))]
    fail2 <- as.matrix( pooled[indices2, grepl("fails.", names(pooled))] )
    fail2 <- fail2[,(t:(K+1))]
    A2 <- as.matrix( pooled[indices2, grepl("A.", names(pooled))] )
    V2 <- as.matrix( pooled[indices2, grepl("V", names(pooled))] )
    La2 <- as.matrix( pooled[indices2, grepl("L.La.", names(pooled))] )
    Lb2 <- as.matrix( pooled[indices2, grepl("L.Lb.", names(pooled))] )
    MLb2 <- as.matrix( pooled[indices2, grepl("MLb.", names(pooled))] )
    ...
    id3 <- CC$rowname[indices3]
    
    if (MISSTYPE=="gaussian"){
      if (t > 1){
        tmpMLb3 <- MLb3[,(1:(t-1))]
        tmpLb3 <- Lb3[,(1:(t-1))]
        tmpLb3[is.na(tmpLb3)] <- 0
        if (Mimp==T){
          imp.model <- geeglm(outLb3~ 1 + tmpA3 + tmpV3 + tmpLa3 + tmpLb3 + tmpMLb3, weights=wLb3, id=id3, family=gaussian)
          coef <- imp.model$coef
          var <- as.numeric(summary(imp.model)$dispersion[1])
          Xmatmain <- as.matrix(cbind(1,tmpA2,tmpV2,tmpLa2,tmpLb2,tmpMLb2))
        } else {
          imp.model <- geeglm(outLb3~ 1 + tim3 + fail3 + tmpA3 + tmpV3+ tmpLa3 + tmpLb3 + tmpMLb3, weights=wLb3, id=id3, family=gaussian)
          coef <- imp.model$coef
          var <- as.numeric(summary(imp.model)$dispersion[1])
          Xmatmain <- as.matrix(cbind(1,tim2,fail2,tmpA2,tmpV2,tmpLa2,tmpLb2,tmpMLb2))
        }
      } else {
        if (Mimp==T){
          imp.model <- geeglm(outLb3~ 1 + tmpA3 + tmpV3 + tmpLa3, weights=wLb3, id=id3, family=gaussian)
          coef <- imp.model$coef
          var <- as.numeric(summary(imp.model)$dispersion[1])
          Xmatmain <- as.matrix(cbind(1,tmpA2,tmpV2,tmpLa2))
        } else {
          imp.model <- geeglm(outLb3~ 1 + tim3 + fail3 + tmpA3 + tmpV3 + tmpLa3, weights=wLb3, id=id3, family=gaussian)
          coef <- imp.model$coef
          var <- as.numeric(summary(imp.model)$dispersion[1])
          Xmatmain <- as.matrix(cbind(1,tim2,fail2,tmpA2,tmpV2,tmpLa2))
        }
      }
    ...
    if (MISSTYPE=="binomial"){
      if (t > 1){
        ...
        tmpLb3[is.na(tmpLb3)] <- 0
        if (Mimp==T){
          imp.model <- geeglm(outLb3~ 1 + tmpA3 + tmpV3 + tmpLa3 + tmpLb3 + tmpMLb3, weights=wLb3, id=id3, family=binomial)
          coef <- imp.model$coef
          Xmatmain <- as.matrix(cbind(1,tmpA2,tmpV2,tmpLa2,tmpLb2,tmpMLb2))
        } else {
          imp.model <- geeglm(outLb3~ 1 + tim3 + fail3 + tmpA3 + tmpV3 + tmpLa3 + tmpLb3 + tmpMLb3, weights=wLb3, id=id3, family=binomial)
          coef <- imp.model$coef
          Xmatmain <- as.matrix(cbind(1,tim2,fail2,tmpA2,tmpV2,tmpLa2,tmpLb2,tmpMLb2))
        }
      } else {
        if (Mimp==T){
          imp.model <- geeglm(outLb3~ 1 + tmpA3 + tmpV3 + tmpLa3, weights=wLb3, id=id3, family=binomial)
          coef <- imp.model$coef
          Xmatmain <- as.matrix(cbind(1,tmpA2,tmpV2,tmpLa2))
        } else {
          imp.model <- geeglm(outLb3~ 1 + tim3 + fail3 + tmpA3 + tmpV3 + tmpLa3, weights=wLb3, id=id3, family=binomial)
          coef <- imp.model$coef
          Xmatmain <- as.matrix(cbind(1,tim2,fail2,tmpA2,tmpV2,tmpLa2))
        }
      }
  ...
  for (m in 0:K) {
    for (k in (K-m):0) {
      ...
      # only used by Method 2.
        ...
        imp.Lb <- milist[[k+1]]
        imp.Lb <- subset(imp.Lb,select=c("id","itr","Lb"))
        names(imp.Lb) <- c("pseudoi.use", "itr","imp.Lb")
        pooled <- left_join(pooled,imp.Lb,by=c("pseudoi.use","itr"))
        pooled1 <- pooled[, grep("L.Lb", names(pooled),invert = TRUE)] 
        pooled2 <- pooled[, grepl("L.Lb", names(pooled))]
        
        if (k > 0){
        pooled2[,ncol(pooled2)] <- pooled$imp.Lb
        pooled1 <- pooled1[, grep("imp.Lb", names(pooled1),invert = TRUE)] 
        pooled <- cbind(pooled1,pooled2)
        } else {
        pooled$L.Lb0 <- pooled$imp.Lb
        pooled <- pooled[, grep("imp.Lb", names(pooled),invert = TRUE)] 
        }
        pooled <- left_join(pooled,mp.Lb,by=c("pseudoi.use"))
        pooled3 <- as.data.frame(cbind(pooled[, grepl("MLb", names(pooled))][,(k+1)], pooled[, grepl("estimates.mp", names(pooled))]))
        if (ESTIMATOR=="FMI"){
        pooled3 <- transform(pooled3, mp.weight=(1/M))
          } else {
        pooled3 <- transform(pooled3, mp.weight=((V2-abs(V1-1))/(V2*M)))
        }
        ...
        pstmp2 <- rbind(pstmp.CC,pooled)
        }
        pstmp2[is.na(pstmp2)] <- 0
        pstmp2 <- transform(pstmp2, weight=pseudowei.use*pseudocenswei.use*mp.weight)
        
        Y <- pstmp2$Aoutcome
        if (k > 0){
        X <- as.matrix(cbind(1,pstmp2[, grepl("covA|A[^o]", names(pstmp2))],pstmp2[, grepl("V", names(pstmp2))],pstmp2[, grepl("L.La.", names(pstmp2))],pstmp2[, grepl("L.Lb.", names(pstmp2))],pstmp2[, grepl("MLb.", names(pstmp2))][,useML[k+1,]==T],pstmp2[, grepl("currt", names(pstmp2))]))
        } else {
      ...
      tim2 <- as.matrix(data3$tim)
      tau2 <- as.matrix( data3[, grepl("tau.", names(data3))] )
      tim.diff2 <- pmin(tim2, tau2[, k+m+2]) - tau2[, k+m+1]
      fail2 <- data3$fail
      A2 <- as.matrix( data3[, grepl("A.A.", names(data3))] )
      V2 <- as.matrix( data3[, grepl("V", names(data3))] )
      L2 <- as.matrix( data3[, grepl("L.L", names(data3))] )
      MLb2 <- as.matrix( data3[, grepl("MLb.", names(data3))] )
     ...
     deltastar2 <- array(0, c(nrow(data3), K+1, K+1))
      ...
      if (m>1)
        for (kindex in (k+1):(k+m-1))
          for (lindex in kindex:(k+m-1)) {
            ...
            logwei.base2 <- logwei.base2 + deltastar2[, kindex+1, k+1] * Zpsi * ( tau2[, lindex+2] - tau2[, lindex+1] )
          }
      ...
      atrisk2 <- tim2 >= tau2[, k+m+1]
      fail.in.interval2 <- tim2 >= tau2[, k+m+1] & tim2 < tau2[, k+m+2] & fail2==T
        ...
        if (sum(useA[k+1,])>0) {
          ...
          covA2 <- NULL
        if (sum(useL[k+1,])>0) {
          covL2 <- as.data.frame(L2)
          covMLb2 <- as.data.frame(MLb2)[,useML[k+1,]==T]
        } else
          covL2 <- NULL        
        ...
        if (k>0){
        covars2 <- cbind(1, covA2, covV2, as.matrix(covL2[, grepl("L.La", names(covL2))]), as.matrix(covL2[, grepl("L.Lb", names(covL2))]), as.matrix(covMLb2), prevt2)
        } else {
        covars2 <- cbind(1, covA2, covV2, as.matrix(covL2[, grepl("L.La", names(covL2))]), as.matrix(covL2[, grepl("L.Lb", names(covL2))]), prevt2)
        }  
        ...
        if (EXPOSETYPE=="gaussian"){
          delta.base <- A2[, k+1] - cbind(covars2,0)%*%fitA.tim
          delta.tim <- A2[, k+1] - cbind(covars2,tim.diff2)%*%fitA.tim
        }
        if (EXPOSETYPE=="binomial"){
          delta.base <- A2[, k+1] - expit(cbind(covars2,0)%*%fitA.tim)
          delta.tim <- A2[, k+1] - expit(cbind(covars2,tim.diff2)%*%fitA.tim)
        }
       ...
       if (DIMZ==1) {
        ...
        if (EXPOSETYPE=="binomial")
          integ <- integral.logit(b=deltastarZpsi.km2 + censrate.est2[, k+m+1] - censrate.stab2[, k+1, k+m+1], a=A2[,k+1], e=logit(A2[,k+1]-delta.base), d=Z.currtcoeff, y=tim.diff2)
        ...
        numer1.ind.woz[fail.in.interval2] <- ( exp(logwei.tim2 + logcenswei.tim2) * delta.tim * dlweight)[fail.in.interval2==T]
        numer1 <- rep(0, DIMZ)
        for (zj in 1:DIMZ)
          numer1[zj] <- sum( (numer1.ind.woz * Z2[, k+1, k+m+1, zj])[fail.in.interval2==T] )

        # numer2 is (minus) the second part of the numerator in the estimator of psiA.
        if (m>0)
          numer2.ind.woz[atrisk2==T] <- ( sum.delpsiZ2 * integ * dlweight)[atrisk2==T]
        numer2 <- rep(0, DIMZ)
        for (zj in 1:DIMZ)
          numer2[zj] <- sum(numer2.ind.woz[atrisk2==T] * Z2[atrisk2==T, k+1, k+m+1, zj])
        ...
        denom.ind.woz[atrisk2==T] <- (integ * delta.base * dlweight)[atrisk2==T]          
        if (DIMZ==1) {
          denom[k+1, k+m+1, 1, 1] <- sum( (denom.ind.woz * Z2[, k+1, k+m+1, 1]^2)[atrisk2==T] )
