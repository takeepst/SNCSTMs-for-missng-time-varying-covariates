###########################################################################
#Program name           : generate_data_m.r
...
for (k in 1:K) {
  if (MISSTYPE=="gaussian"){
  L[, k*2+(1:2)] <- mvrnorm(N, mu[k*2+(1:2), k+1], Lvar) + L[, (k-1)*2+(1:2)] %*% alpha[k*2+(1:2), (k-1)*2+(1:2)] + A[, 1:k] %*% t(beta[k*2+(1:2), 1:k]) + 0.5*U[,1]- 0.2*V[,1]
  }
  if (MISSTYPE=="binomial"){
  L[, k*2+(1:2)] <- mvrnorm(N, mu[k*2+(1:2), k+1], Lvar) + L[, (k-1)*2+(1:2)] %*% alpha[k*2+(1:2), (k-1)*2+(1:2)] + A[, 1:k] %*% t(beta[k*2+(1:2), 1:k]) + 0.5*U[,1] - 0.2*V[,1]
  pL2[, k+1] <- (exp(-0.5+log(1.5)*A[,k]+log(1.2)*L[,k*2+1]+log(0.8)*L[,(k-1)*2+2]+0.5*U[,1]-0.2*V[,1])/(1+exp(-0.5+log(1.5)*A[,k]+log(1.2)*L[,k*2+1]+log(0.8)*L[,(k-1)*2+2]+0.5*U[,1]-0.2*V[,1])))
  L[, k*2+2] <- rbinom(N,1,pL2[, k+1])
  }
  if (EXPOSETYPE=="gaussian"){
    A[, k+1] <- rnorm(N, mean=2+0.5*A[,k]+L[,k*2+1]/10 + L[,k*2+2]/20 + V[,1]/5, sd=0.7)
  }
  if (EXPOSETYPE=="binomial"){
    pA[, k+1] <- (exp(-0.5+log(3.0)*A[,k]+log(1.5)*L[,k*2+1]+0.8*L[,k*2+2]-0.2*V[,1])/(1+exp(-0.5+log(3.0)*A[,k]+log(1.5)*L[,k*2+1]+0.8*L[,k*2+2]-0.2*V[,1])))
  ...
  Mprob[, k+1] <- (exp(delta00+deltaA*A[,k+1]+deltaV*V[,1]+deltaL*L[,(2*k)+1]+gammaL*L[,(2*k)+2]+deltaF*fails[,k+1]+deltaT*tims[,k+1])/(1+exp(delta00+deltaA*A[,k+1]+deltaV*V[,1]+deltaL*L[,(2*k)+1]+gammaL*L[,(2*k)+2]+deltaF*fails[,k+1]+deltaT*tims[,k+1])))
